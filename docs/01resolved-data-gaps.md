# 01Resolved data coverage in 01r.trade

This is the source-of-truth inventory for the 01Resolved-only cutover. A row is
available only when 01r.trade has a reviewed server contract for it. Missing rows
must remain visibly unavailable; they must not be synthesized or restored from
a fallback provider.

## Available now

| 01r.trade surface | Source | Contract used by 01r.trade |
| --- | --- | --- |
...
chart provider. 01r.trade discovers the DAO and mints from program-owned proposal
...
| Needed 01r.trade data | Current behavior | Product impact |
| --- | --- | --- |
...
On 2026-08-02, a GET-only check through the local 01r.trade preview returned 31
...
treasury-overview endpoints instead. 01r.trade now uses the global endpoint only as
...
01Resolved or 01r.trade deployment.
